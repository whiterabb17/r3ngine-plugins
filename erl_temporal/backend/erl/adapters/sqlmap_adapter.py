...
class SqlmapAdapter(BaseAdapter):
    ...
    def validate(self, finding: Finding) -> Dict[str, Any]:
        ...
        cmd = f"-u \"{finding.url}\" --batch --level 1 --risk 1"
        
        # OpSec: User-Agent
        if self.opsec_manager.is_enabled() and self.opsec_manager.settings.enable_random_ua:
            cmd += f" --user-agent=\"{self.opsec_manager.get_random_ua()}\""
        else:
            cmd += " --random-agent"

        # Proxy Support
        # If proxychains is enabled, BaseAdapter handles it. 
        # If not, but proxies are available, use sqlmap's native --proxy flag.
        if not self.proxy_manager.should_wrap() and self.proxy_manager.proxies:
            proxy_str = self.proxy_manager.get_random_proxy()
            parts = proxy_str.split()
            if len(parts) >= 3:
                proto, host, port = parts[0], parts[1], parts[2]
                cmd += f" --proxy=\"{proto}://{host}:{port}\""
