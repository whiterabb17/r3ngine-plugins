from celery.utils.log import get_task_logger
from reNgine.celery_custom_task import RengineTask
from reNgine.celery import app
import os
import sys

logger = get_task_logger(__name__)

@app.task(name='erl_celery_run', base=RengineTask, bind=True)
def run(self, ctx):
    """
    Plugin-based ERL validation task.
    """
    scan_history_id = ctx.get('scan_history_id')
    if not scan_history_id:
        logger.error("ERL Plugin: No scan_history_id in context")
        return
        
    logger.info(f"ERL Plugin: Starting validation for scan {scan_history_id}")
    
    # Add current backend dir to sys.path to ensure erl.core.orchestrator is findable
    # The orchestrator.py already does this, but being explicit doesn't hurt
    backend_dir = os.path.dirname(os.path.abspath(__file__))
    if backend_dir not in sys.path:
        sys.path.append(backend_dir)
        
    try:
        from erl.core.orchestrator import Orchestrator
        orchestrator = Orchestrator()
        orchestrator.process_scan(scan_history_id)
        logger.info(f"ERL Plugin: Completed validation for scan {scan_history_id}")
    except Exception as e:
        logger.error(f"ERL Plugin: Failed validation: {str(e)}")
        raise e
